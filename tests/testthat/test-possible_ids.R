# PREPARATION ####
withr::local_options(joyn.verbose = FALSE, possible_ids.verbose = FALSE)
library(data.table)
...
y1 = data.table(id = c(1, 2, 4), y = c(11L, 15L, 16))
...
y3 <- data.table(id = c("c", "b", "c", "a"), y = c(11L, 15L, 18L, 20L))

x3 <- data.table(id = c("c", "b", "d"), v = 8:10, foo = c(4, 2, 7))
...
generate_random_data <- function(n, type) {
  switch(
    ...
    "datetime" = as.POSIXct("2000-01-01") +
      sample(0:(3650 * 24 * 60 * 60), n, replace = TRUE)
  )
}
...
test_that("create_ids works as intended", {
  ...
  # ret class
  res <- create_ids(n_rows = 50, n_ids = 1)
  ...
  res <- dt[, (vars) := create_ids(.N, n_ids = 3)]
  ...
  new_ids <- as.data.table(create_ids(nrow(dt), n_ids = 3, prefix = "new_id"))
...
test_that("store checked ids", {
  res <- possible_ids(x4, get_all = TRUE)
  ...
  res <- possible_ids(dt, vars = paste0("numeric_int_", 1:4))
  ...
  res <- possible_ids(dt, vars = paste0("numeric_int_", 1:4), get_all = TRUE)
...
test_that("vars provided by user", {
  ...
  possible_ids(x4, vars = c("t")) |>
    expect_error()

  # one or more vars not included in dt
  possible_ids(x4, vars = c("id3", "id2")) |>
    expect_error()

  possible_ids(dt, vars = c("id", "id3", "id2")) |>
    expect_error()
  ...
  use_ids_dt <- possible_ids(dt, vars = vars)

  all(sapply(use_ids_dt, function(x) {
    x %in% ids_dt
  })) |>
    expect_equal(TRUE)
  ...
  if (res == 0) {
    ...
    possible_ids(dt, vars = vars, min_combination_size = 3) |>
      unlist() |>
...
test_that("relationship include and vars", {
  possible_ids(x4, vars = c("id1", "id2"), include = c("t")) |>
    expect_no_error()
  ...
  possible_ids(x4, include = c("t", "x")) |>
    expect_no_error()

  possible_ids(x4, vars = c("t", "x"), include = NULL) |>
    expect_no_error()

  # test checked vars are at least `vars` plus those in `include`
  res <- possible_ids(x4, vars = c("id1", "id2", "t"), include = "t")
...
test_that("exclude and include", {
  ...
  res <- possible_ids(
    dt,
    include = c("numeric_double_1", "numeric_double_2"),
  ...
  res_ids <- possible_ids(x2, include = "x", exclude_classes = "numeric")

  res_ids |>
    unlist() |>
    expect_equal(c("t", "x"))

  res_ids <- possible_ids(x3, include = "id", exclude_classes = "character")
  ...
  possible_ids(dt, include_classes = "numeric", exclude_classes = "numeric") |>
    expect_error()
  ...
  attributes(res)$checked_ids |>
    expect_setequal(setdiff(
      names(dt),
      c(paste0("numeric_int_", 2:10), "id", "unique_id1", "unique_id3")
    ))
})
...
test_that("get all works", {
  # no error
  possible_ids(x4, get_all = TRUE) |>
    expect_no_error()

  # get all with user selected vars
  possible_ids(x4, vars = c("id1", "t"), get_all = TRUE) |>
    expect_no_error()

  # get all with max number of combinations
  possible_ids(x4, max_combination_size = 3, get_all = TRUE) |>
    expect_no_error()

  # check get all combs
  possible_ids(x3, get_all = TRUE) |>
    unlist() |>
...
test_that("Max combination size", {
  ...
  res <- possible_ids(x1, get_all = TRUE, max_combination_size = 2)
...
test_that("Min combination size", {
  res <- possible_ids(x4, min_combination_size = 1, get_all = FALSE) |>
    unlist()
...
test_that("Exclude type and variable", {
  ...
  dd <- possible_ids(xx4, exclude_classes = c("character"), exclude = "x")
...
test_that("Exclude more than one variable", {
  dd <- possible_ids(x4, exclude = c("id2", "x"))
...
test_that("identifies ids", {
  ...
  possible_ids(dt, vars = vars) |>
    unlist() |>
    expect_equal(vars)

  df_test <- as.data.frame(
    create_ids(n_rows = 50, n_ids = 3)
  )

  possible_ids(df_test, vars = c("id1", "id2"), include = "id3") |>
    unlist() |>
